...
const mockStudents = [
  { id: 's1', name: 'Nguyễn Văn A', studentId: 'SV01' },
  { id: 's2', name: 'Trần B', studentId: 'SV02' },
  { id: 's3', name: 'Lê C', studentId: 'SV03' },
]
...
const examResults = computed(() =>
  mockResults.filter(r => r.examId === selectedExam.value)
)

const avgScore = computed(() => {
  if (!examResults.value.length) return '0.00'
  const total = examResults.value.reduce((sum, r) => sum + r.score, 0)
  return (total / examResults.value.length).toFixed(2)
})
  ...
  <div>
    ...
    <div class="mb-6 bg-white p-6 rounded shadow">
      ...
      <select v-model="selectedExam" class="border p-2 rounded ml-2">
        <option v-for="e in mockExams" :key="e.id" :value="e.id">
          {{ e.title }}
        </option>
      </select>
    </div>
    ...
    <div class="bg-white p-6 rounded shadow mb-6">
      ...
      <div class="flex items-end justify-around h-40 gap-4">
        <div
          v-for="(count, idx) in scoreRanges"
          :key="idx"
          class="w-full flex flex-col items-center"
        >
          <div
            class="w-full bg-blue-500 rounded-t"
            :style="{ height: `${count * 20 + 5}%` }"
          />
    ...
    <div class="bg-white rounded shadow overflow-hidden">
      <table class="w-full">
        <thead class="bg-gray-50 border-b">
          <tr>
            <th class="p-4 text-left">Sinh viên</th>
        ...
        <tbody>
          ...
          >
            <td class="p-4">{{ getStudentName(r.studentId) }}</td>
            <td class="p-4 font-bold">{{ r.score }}</td>
            <td class="p-4 text-right">
              <button
                @click="selectedResult = r; showAnswersModal = true"
                class="text-blue-600 flex items-center gap-1 justify-end w-full"
              >
    ...
    <!-- Modal -->
    <div
      v-if="showAnswersModal && selectedResult"
      class="fixed inset-0 bg-black/50 flex items-center justify-center z-50"
    >
      <div class="bg-white p-6 rounded w-96">
        <h3 class="font-bold text-lg mb-4">Kết quả chi tiết</h3>
        <p>Sinh viên: {{ getStudentName(selectedResult.studentId) }}</p>
